...
- **複数辞書の重ね掛け・切り替え**（最大5つ・優先度の高い辞書から順に候補を表示）
- **読み → 候補の変換ポップアップ**
...
3. **初回だけ**、下の手順で起動を許可してください（署名がないため、macOS が初回のみブロックします）。

> **⚠️「マルウェアが含まれていないことを検証できませんでした」と出たら**
>
> このダイアログの青いボタンは **「ゴミ箱に入れる」** です。**押さないでください。**
>
> **macOS 15（Sequoia）以降・26（Tahoe）の場合**
>
> 1. ダイアログの **「完了」** を押して閉じます。
> 2. **システム設定 → プライバシーとセキュリティ** を開きます。
> 3. 下へスクロールし、**「セキュリティ」** 欄にある「"Kotobagae" は…ブロックされました」の横の **「このまま開く」** をクリックします。
> 4. Touch ID かパスワードで認証し、確認ダイアログで **「開く」** を押します。
>
> **macOS 14（Sonoma）以前の場合**
>
> アプリを **右クリック →「開く」** で起動できます。
>
> どちらも **初回だけ** です。次回からは普通にダブルクリックで起動します。
...
- **最大5つ**まで重ねられ、**優先度の高い辞書から順に、候補がまとめて並びます**。
...
**Q.（Mac）「マルウェアが含まれていないことを検証できませんでした」と出て、「ゴミ箱に入れる」しか押せません。**
A. 署名がないアプリに macOS が出す、初回だけの警告です。**「ゴミ箱に入れる」は押さず「完了」** を押し、**システム設定 → プライバシーとセキュリティ →「このまま開く」** から起動を許可してください（macOS 15 以降。`右クリック →「開く」` での回避は macOS 15 で廃止されました）。詳しい手順は上の「ダウンロード / インストール → macOS」をご覧ください。
...
- ソースコードは公開していますが、**再利用のためのライセンスは付与していません**（著作権は作者に帰属します）。外部からのコントリビューション（プルリクエスト等）は当面受け付けていません。
